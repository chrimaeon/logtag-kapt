...
class LogTagProcessorShould {
    ...
    fun `generate code for valid kotlin annotation`() {
        ...
        whenever(roundEnvironmentMock.getElementsAnnotatedWith(any<Class<out Annotation>>())) doReturn
            mutableSetOf(
                element,
            )
    ...
    fun `truncate tag if class name is too long`() {
        ...
        val expected =
            ...
            public inline val TestClassWithAFarTooLongName.LOG_TAG: String
              get() = "TestClassWithAFarTooLon"
...
private fun String.asName(): Name =
    object : Name {
        ...
        override fun subSequence(
            startIndex: Int,
            endIndex: Int,
        ): CharSequence = this@asName.subSequence(startIndex, endIndex)

        override fun contentEquals(cs: CharSequence?): Boolean = this@asName == cs

        override val length: Int
            get() = this@asName.length

        override fun toString(): String = this@asName

        override fun equals(other: Any?): Boolean = this@asName == other
    }
